...
class fflock_encoder(threading.Thread):
    def __init__(self, inpath, outpath, preoptions, codecSettings, encoder, overwrite, extraArgs=" "):
        ...
        self.args = "%s %s %s -analyzeduration 1000000 -i %s %s %s %s" % (encoder,
                                                                          strOverwrite, preoptions, inpath,
                                                                          codecSettings, extraArgs, outpath)
        print self.args
    ...
    def run(self):
        ...
        self.output = []        #all lines of output from encoder
        self.durationFlt = -1    #duration of audio in seconds
        partialLine = ""        #currnt console line which is not complete

        # read a set amount of characters from encoder's console output in order to build entire lines of output
        while True:
